...
| `agents/<name>.json` | Agent configurations. The triage agent lives at `agents/openit-triage.json`. |
...
The triage agent's persona lives at `agents/openit-triage.json` (`name`, `description`, `instructions`). It's invoked by the `ai-intake` skill — see Skills below — once per turn the chat-intake server runs. Edit the `instructions` field there to tweak how the agent talks to end users; that's the source of truth for the agent's voice.

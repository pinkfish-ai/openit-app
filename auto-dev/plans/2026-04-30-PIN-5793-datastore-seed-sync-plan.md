...
### Cloud-side bug discovered + fixed during stage-04

Stage-04 integration testing surfaced a sixth cloud-side bug not on the original `2026-04-29-datastore-cloud-fixes.md`: **`POST /memory/items` rejected rows on structured collections that were created with caller-supplied semantic field IDs.** The schema POST honored fields like `firstName` (cloud fixes #1 + #2 already deployed), and `GET /datacollection/{id}` echoed the schema back with those IDs intact — but row-insert errored with `Unknown column: 'firstName'`. Root cause in `firebase-helpers/utils/schema-resolver.ts::buildSchemaMapping`: the column→fieldId map only registered `field.label`, never `field.id`, so semantic-id row content had no entry in the resolver and got rejected.

Fixed at the source:

- **firebase-helpers PR #462** — adds `field.id` (and lowercase) entries to the column→fieldId map alongside the existing label entries. Internal `f_N` ids hit a short-circuit higher up so the addition is a no-op there. Coverage: 4 new `content-converter.test.ts` cases.
- **platform PR #3287** — `TestPostMemoryItemsAcceptsRowKeyedBySemanticFieldIds` Go integration test against `dev20` validates the round trip + mixed shapes + unknown-column-still-rejected guard.

In **openit-app PR #74**, this means tickets+people stay **structured** with the bundled schema posted in the create body (matches the brief intent — `_schema.json` is the contract). Conversations stays unstructured. The integration scenario test soft-skips the structured-row-insert assertions when the cloud probe shows the deploy hasn't landed yet — auto-create + unstructured-row push assertions still run unconditionally. Once firebase-helpers#462 deploys to `dev20`, the structured assertions activate automatically; no openit-app code change required.
